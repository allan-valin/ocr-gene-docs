...
Indices are **sequential**, so the index range approximates the date range — there is no
date field to filter on at catalog time. The saved index pages were chosen to bound the
...
inference stack, applied to the client. No framework-version floor, no WebGL, no
browser-specific APIs. The prototype is checked in Chromium and Firefox on every build
(`scripts/smoke_prototype.py`), and both must pass before a change counts as done.
...
- **Verified functionally, not just visually.** `scripts/smoke_prototype.py` drives the
  real page in headless Chromium and in Firefox through geckodriver's WebDriver API,
  asserting 19 behaviours: rendering, band painting, each search scope, fuzzy and variant
  matching, bidirectional scroll sync, the lock toggle, the name-column crop, verification,
  and the null/ditto rendering. An earlier build rendered perfectly and did nothing,
  because `fetch()` is blocked on `file://` URLs — screenshots alone would not have caught
  it, so data is now embedded at build time.

### Search scoping

Searching every column at once is wrong for this corpus: looking for a passenger named
*Paul* returns everyone living in *São Paulo*, because addresses sit in Observações. The
find widget therefore has a **column dropdown defaulting to Nome**, with an entry per
column plus "todas as colunas". Fuzzy matching is tuned against real pairs from the
sample — `vasquez`/`vazquez` scores 0.455 on trigram similarity and must match, while
scope (not threshold) is what keeps *Paul* away from *São Paulo*.

### Loading other documents

A `file://` page cannot list a directory or read arbitrary files, so the document list is
baked in at build time from the download manifest: `scripts/make_prototype.py` renders the
first N pages of each dossier and embeds the index. The sidebar lists ship, notation and
page count, badges the transcribed one, and a page strip switches pages within a dossier.
Untranscribed documents show their scan with an explicit "documento não transcrito"
message — never invented rows. In the real app this list comes from the database and the
constraint disappears.

### Interface language

The UI is pt-BR. The users are the friend doing jus sanguinis work and Brazilian archive
researchers, and the domain vocabulary is Portuguese. Note the *document's own* column
header reads "Nome e Cognomes" — archaic, Italian-influenced; the app uses modern
"Nome" and "Estado civil" rather than mirroring the form's wording.
