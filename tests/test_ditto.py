...
def rows(*texts):
    # what the engine writes since T6: the reading, and nothing claiming which
    # part of it is a family name
    return [{"n": i, "name_raw": t} for i, t in enumerate(texts, 1)]


def repeats(row):
    """The words this row takes from the one above it, as one string."""
    return " ".join(row.get("inherited") or [])


def test_a_mark_under_a_surname_means_that_surname():
    out = resolve(rows("Martinez Francisco", '" Maria'))
    assert repeats(out[1]) == "Martinez"
    assert out[1]["name_raw"] == '" Maria'
    assert out[1]["ditto"] == ["name"]
...
def test_the_mark_is_whatever_the_recogniser_made_of_it():
    ...
    for mark in ('"', "''", ",,", "11", "//", "”", "«", ".", "-", "n"):
        ...
        assert repeats(out[1]) == "Lorenzo", mark
...
def test_a_name_is_not_a_mark():
    ...
    assert "ditto" not in out[1] and "inherited" not in out[1]
    assert "ditto" not in out[2] and "inherited" not in out[2]


def test_a_mark_with_nothing_above_it_inherits_nothing():
    out = resolve(rows('" Maria', "Martinez Francisco"))
    assert "inherited" not in out[0]
...
def test_the_mark_carries_down_a_whole_family():
    ...
    assert [repeats(r) for r in out] == ["", "Santa", "Santa", "Santa",
                                         "", "Arquentiri"]
...
def test_the_engine_s_own_rows_keep_their_other_fields():
    out = resolve([{"n": 1, "name_raw": "Martinez Francisco",
                    "conf": {"name": 0.8}},
                   {"n": 2, "name_raw": '" Maria',
                    "conf": {"name": 0.4}, "alternatives": ["x"]}])
    assert out[1]["conf"] == {"name": 0.4}
    assert out[1]["alternatives"] == ["x"]


def indented(text, indent):
    return {"name_raw": text, "indent": indent}
...
def test_a_row_written_under_the_mark_is_a_continuation_even_unmarked():
    ...
    out = resolve([indented("Santabarbara Salvador", 0.02),
                   ...
                   indented("Maria", 0.31)])
    assert [repeats(r) for r in out] == ["", "Santabarbara", "Santabarbara"]
    assert out[1]["name_raw"] == "Felipe" and out[1]["ditto"] == ["name"]
