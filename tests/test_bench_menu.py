"""The menu bench: does the true name reach the person, and at what rank.
...
truth reports improvements that are not there.
"""
import runpy
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
bench = runpy.run_path(str(ROOT / "scripts" / "bench_menu.py"))
...
def test_a_measurement_counts_recall_at_each_depth_over_the_bad_words_only():
    ...
    m = bench["measure"](cases, candidates)
...
def test_a_measurement_over_no_bad_words_reports_zeroes_rather_than_dividing_by_zero():
    m = bench["measure"]([{"truth": "MARIA", "read": "MARIA", "i": 0, "row": {}}],
                         lambda w, r, i: [])
...
def test_the_engine_alternates_of_a_row_are_a_candidate_source():
    ...
    assert bench["alts_for"]("FORE", row, 0) == ["fose"]
    assert bench["alts_for"]("GULERTI", row, 1) == []
    assert bench["alts_for"]("ANYTHING", {}, 0) == []
